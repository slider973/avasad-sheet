...
class PdfProcessorEndpoint extends Endpoint {
  ...
  Future<String> _regeneratePdfWithSignature(
    ...
    try {
      ...
      final newFileName = validation.pdfPath.replaceAll('.pdf', '_signed_${DateTime.now().millisecondsSinceEpoch}.pdf');
  ...
  Future<void> cleanupOldJobs(Session session) async {
    try {
      ...
      final oldJobs =
          allCompletedOrFailed.where((job) => job.createdAt != null && job.createdAt!.isBefore(cutoffDate)).toList();
