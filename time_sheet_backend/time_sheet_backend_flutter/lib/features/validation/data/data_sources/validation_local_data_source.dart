...
class ValidationLocalDataSourceImpl implements ValidationLocalDataSource {
  ...
  Future<ValidationRequestModel?> getValidationRequest(String id) async {
    final cache = await isar.validationRequestCaches.where().validationIdEqualTo(id).findFirst();
  ...
  Future<List<ValidationRequestModel>> getEmployeeValidations(String employeeId) async {
    final caches =
        await isar.validationRequestCaches.where().employeeIdEqualTo(employeeId).sortByLastUpdatedDesc().findAll();
  ...
  Future<List<ValidationRequestModel>> getManagerValidations(String managerId) async {
    final caches =
        await isar.validationRequestCaches.where().managerIdEqualTo(managerId).sortByLastUpdatedDesc().findAll();
  ...
  Future<void> deleteValidationRequest(String id) async {
    await isar.writeTxn(() async {
      await isar.validationRequestCaches.where().validationIdEqualTo(id).deleteAll();
    });
  }
  ...
  Future<List<NotificationModel>> getUserNotifications(String userId) async {
    final caches = await isar.notificationCaches.where().userIdEqualTo(userId).sortByCreatedAtDesc().findAll();
  ...
  Future<NotificationModel?> markNotificationAsRead(String notificationId) async {
    final cache = await isar.notificationCaches.where().notificationIdEqualTo(notificationId).findFirst();
  ...
  Future<void> markAllNotificationsAsRead(String userId) async {
    final caches = await isar.notificationCaches.where().userIdEqualTo(userId).filter().isReadEqualTo(false).findAll();
  ...
  Future<int> getUnreadNotificationCount(String userId) async {
    return await isar.notificationCaches.where().userIdEqualTo(userId).filter().isReadEqualTo(false).count();
  }
  ...
  Future<List<Map<String, dynamic>>> getPendingSyncItems() async {
    final items = await isar.syncQueueItems.where().filter().syncedEqualTo(false).sortByCreatedAt().findAll();
