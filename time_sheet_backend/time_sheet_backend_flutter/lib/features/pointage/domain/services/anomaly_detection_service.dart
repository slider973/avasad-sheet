...
class AnomalyDetectionService {
  ...
  Future<Map<String, List<AnomalyResult>>> detectAnomaliesWithWeeklyCompensation(
    ...
    final compensationConfig = _globalRuleConfigs['weekly_compensation'] ?? compensationRule.defaultConfiguration;
    ...
    for (final weekEntry in weekGroups.entries) {
      ...
      for (final entry in weekEntries) {
        final entryId = entry.id?.toString() ?? '';
        final entryAnomalies =
            compensatedWeekAnomalies.where((anomaly) => anomaly.timesheetEntryId == entryId).toList();
  ...
  void importConfiguration(Map<String, dynamic> config) {
    if (config.containsKey('ruleConfigs')) {
      ...
      for (final entry in ruleConfigs.entries) {
        _globalRuleConfigs[entry.key as String] = Map<String, dynamic>.from(entry.value as Map);
      }
    }
